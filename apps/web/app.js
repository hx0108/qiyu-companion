...
function renderAge() {
  ...
  return prototypeShell(`<div class="content"><div class="eyebrow">Age assurance</div><h1 id="app-title">年龄保障</h1><p class="lead">先完成基础年龄判断。只有出现账号或行为风险信号时，才会请求更强的核验。</p><section class="verify-card"><h3>${status === "AGE_PASS" ? "核验已通过" : "基础信息"}</h3><p class="page-sub">${detail}</p>${showBirthInput ? `<label class="field"><span>出生日期</span><input form="age-form" name="birth_date" type="date" required></label>` : ""}<div class="status-row"><span class="status-dot ${status === "AGE_PASS" ? "pass" : ""}"></span>${escapeHtml(status)} · ${escapeHtml(reasons.join("、") || "等待服务端结果")}</div></section><div class="rights">${prototypeIcon("lock", 17)}<span>栖语不保存证件原图；增强年龄核验的供应商接入与结果以服务端状态为准。</span></div></div><div class="bottom-action">${form}<div class="note">浏览器不自行判定年龄结果，也不展示原型模拟的核验成功。</div></div>`, "02 / 03");
}
