  ...
  static async load(client, accountId, { lockDailyUsage = false } = {}) {
    ...
    const queries = [
      ...
      [`SELECT m.message_id, m.conversation_id, m.actor, convert_from(m.content_ciphertext, 'UTF8') AS text,
        m.provider, m.model_version, m.ai_generated, m.world_state_id, m.world_state_version, m.attachments, m.created_at, m.retention_expires_at FROM messages m JOIN conversations c ON c.conversation_id = m.conversation_id
...
async function persistMessage(client, item, exists, retentionDays) {
  if (exists) return;
  return client.query("INSERT INTO messages (message_id, conversation_id, actor, content_ciphertext, created_at, retention_expires_at, provider, model_version, ai_generated, world_state_id, world_state_version, attachments) VALUES ($1, $2, $3, convert_to($4, 'UTF8'), COALESCE($5::timestamptz, CURRENT_TIMESTAMP), COALESCE($5::timestamptz, CURRENT_TIMESTAMP) + make_interval(days => $6), $7, $8, $9, $10, $11, $12::jsonb)", [item.message_id, item.conversation_id, item.actor, item.text, item.created_at || null, retentionDays, item.provider || null, item.model_version || null, Boolean(item.ai_generated), item.world_state_id || null, item.world_state_version || null, JSON.stringify(item.attachments ?? [])]);
}
