...
class PostgresRequestStore extends DevelopmentStore {
  ...
  static async load(client, accountId, { lockDailyUsage = false } = {}) {
    ...
    const queries = [
      ...
      [`SELECT deletion_job_id, account_id, asset_id, scope, state, revocation_epoch,
        physical_cleanup_state, created_at, note FROM deletion_jobs WHERE account_id = $1 AND deleted_at IS NULL`, [accountId]],
  ...
  async flush() {
    ...
    await syncRows(this.client, this.conversationSummaries, before.conversationSummaries, persistConversationSummary);
    ...
    await syncRows(this.client, this.ocImports, before.ocImports, persistOcImport);
...
async function persistDeletionJob(client, item, exists) { if (!exists) return client.query(`INSERT INTO deletion_jobs (deletion_job_id, account_id, asset_id, scope, state, online_disabled_at, revocation_epoch, physical_cleanup_state, note) VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP, $6, $7, $8)`, [item.deletion_job_id, item.account_id, item.asset_id, item.scope, item.state, item.revocation_epoch, item.physical_cleanup_state, item.note]); }
